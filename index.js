...
async function loadSession() {
  ...
  if (!sessionId) {
    console.log("[SHAVIYA-XMD V3] No SESSION_ID found. Please set it in .env or platform env vars.");
    return false;
  }
...
async function ensureBotFiles() {
  ...
  console.log("[SHAVIYA-XMD V3] Local folders verified.");
  await loadSession();
}
  ...
  conn.ev.on("messages.upsert", async (mkk) => {
    try {
      ...
      if (isCmd && commandText === "restart") {
        ...
        await conn.sendMessage(from, { text: "🔄 *SHAVIYA-XMD V3* is restarting...\n\n_Please wait a few seconds._" }, { quoted: mek });
...
app.get("/", (req, res) => res.send(`💎 SHAVIYA-XMD V3 is Running ✅ | Active sessions: ${activeSessions.size}`));
app.listen(port, () => console.log(`🚀 SHAVIYA-XMD V3 Server running on port ${port}`));
